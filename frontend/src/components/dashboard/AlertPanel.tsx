      ...
      <div className="space-y-2.5 overflow-y-auto pr-1" style={{ maxHeight: 220 }}>
        {alerts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 text-gray-400 gap-2">
            ...
            <p className="text-sm font-medium">No active grid emergencies</p>
            <p className="text-xs text-gray-500">All feeder zones are operating normally</p>
          </div>
        ) : (
          alerts.map((alert, idx) => {
            const zoneTitle = SECTION_NAMES[alert.section_id]?.title ?? `Zone ${alert.section_id}`
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-bold text-electric">{zoneTitle}</span>
