...
async def live_feed(websocket: WebSocket):
    ...
    t = 0
    uptime = 0
    classify_mock = load_mock("classify.json")
    ...
    try:
        while True:
            ...
            for section_id in [1, 2, 3, 4, 5]:
                ...
                # Trigger fault alert when anomaly is high (section 3)
                if reading["anomaly_score"] > settings.ANOMALY_THRESHOLD:
                    classify_mock["triggered_at"] = datetime.now(timezone.utc).isoformat()
                    await websocket.send_text(json.dumps({
                        "type": "fault_alert",
                        "data": classify_mock,
                    }))
